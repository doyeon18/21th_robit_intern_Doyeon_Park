...
v4l2_stream_err usb_cam::start_stream(const m_deviceConfig &config)
{
  ...
  stream_thread = std::thread([this, config]()
    ...
    while (streaming) {
      ...
      // USB 카메라는 드물게 비어 있거나 깨진 MJPEG 버퍼를 돌려줄 수 있다.
      // imdecode에 빈 버퍼를 넘기면 OpenCV 예외로 노드 전체가 종료되므로
      // 해당 프레임만 건너뛴다.
      if (buf.bytesused == 0 || buf.index >= buffers.size()) {
        if (xioctl(m_fd, VIDIOC_QBUF, &buf) == -1) {
          CERR_ENDL("Failed to requeue an empty buffer");
          break;
        }
        continue;
      }
      ...
      cv::Mat img = cv::imdecode(
        cv::Mat(1, buf.bytesused, CV_8UC1, buffers[buf.index]), cv::IMREAD_COLOR);
      if (img.empty()) {
        if (xioctl(m_fd, VIDIOC_QBUF, &buf) == -1) {
          CERR_ENDL("Failed to requeue an invalid image buffer");
          break;
        }
        continue;
      }
