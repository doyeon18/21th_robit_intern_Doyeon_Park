...
PanTiltCamera::on_activate(const rclcpp_lifecycle::State &)
{
  ...
  timer_ = this->create_wall_timer(std::chrono::milliseconds(1000 / (int)fps), [this]()
    ...
    if (camera_ && camera_->streaming) {
      ...
      if (!frame.empty()) {
        // usb_camera.cpp에서 발행용 영상을 640x480으로 줄이므로 CameraInfo도
        // 실제 발행 프레임 크기를 사용해야 한다.
...
void PanTiltCamera::publish_pan_tilt_status()
{
  ...
  control_id = camera_->get_control_by_name("pan");
  const int reported_pan = camera_->get_control(control_id);
  control_id = camera_->get_control_by_name("tilt");
  const int reported_tilt = camera_->get_control(control_id);

  // Some Insta360 Link firmware/kernel combinations occasionally return a
  // malformed absolute-position sample while streaming. Never forward that
  // value to a controller. The node starts at a known 0/0 target and keeps
  // pan/tilt updated whenever it accepts a command, so those targets are a
  // safe fallback until V4L2 reports a valid pair again.
  const bool reported_position_valid =
      pan_range[0] <= reported_pan && reported_pan <= pan_range[1] &&
      tilt_range[0] <= reported_tilt && reported_tilt <= tilt_range[1];
  if (reported_position_valid)
  {
    pan = reported_pan;
    tilt = reported_tilt;
  }
  else
  {
    RCLCPP_WARN_ONCE(
        this->get_logger(),
        "Invalid V4L2 pan/tilt sample (%d, %d); using commanded target (%d, %d)",
        reported_pan, reported_tilt, pan, tilt);
  }

  status_msg.pan = pan;
  status_msg.tilt = tilt;
